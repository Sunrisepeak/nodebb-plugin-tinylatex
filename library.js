...
plugin.init = async (params) => {
	...
	const { setting1, setting2 } = await meta.settings.get('tinylatex');
	if (setting1) {
	 ...
	 * */
	routeHelpers.setupPageRoute(router, '/tinylatex', [(req, res, next) => {
		winston.info(`[plugins/tinylatex] In middleware. This argument can be either a single middleware or an array of middlewares`);
		setImmediate(next);
	}], (req, res) => {
		winston.info(`[plugins/tinylatex] Navigated to ${nconf.get('relative_path')}/tinylatex`);
		res.render('tinylatex', { uid: req.uid });
	});

	routeHelpers.setupAdminPageRoute(router, '/admin/plugins/tinylatex', controllers.renderAdminPage);
};
 ...
 *   curl -X GET \
 *     http://example.org/api/v3/plugins/tinylatex/test \
...
plugin.addRoutes = async ({ router, middleware, helpers }) => {
	...
	routeHelpers.setupApiRoute(router, 'get', '/tinylatex/:param1', middlewares, (req, res) => {
